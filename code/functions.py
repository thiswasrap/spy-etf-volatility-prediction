...
def get_yf_time_series(yf, years, symbol, freq=CBD, fut=False):
    for year in years:
        # Assuming __ts_dates returns start_date, start_file, end_date, end_file, and year
        start_date, start_file, end_date, end_file, year = __ts_dates(year, freq)

        # Download data
        df = yf.download(symbol, start=start_date, end=end_date, progress=False)

        # Adjust column assignment to avoid length mismatch
        if fut:
            df.columns = YF_F_COLUMNS[:len(df.columns)]  # Match the length of the columns
        else:
            df.columns = YF_COLUMNS[:len(df.columns)]  # Match the length of the columns
